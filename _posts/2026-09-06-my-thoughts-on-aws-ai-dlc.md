...
지금도 함께 일하는 동료들은 뛰어나고 좋은 사람들이고, 회사가 주는 안정성과 기회도 크다.
...
컨셉은 어떤 미래를 목표로 하는지다. 프로세스는 그 목표를 향해 일을 어떻게 나누고 반복하는지다. 구현은 그 일하는 방식을 실제 도구와 실행 환경에 어떻게 담는지다.

목표로 삼는 미래에 따라 일하는 방식이 달라지고, 그 방식이 구현의 형태를 결정한다.
...
AI 에이전트는 도구를 사용해 작업을 수행하는 AI다. 나는 이런 에이전트에게 개발 작업을 맡기는 에이전틱 엔지니어링의 방향을 판단할 기준을 이전 글에서 하나 정했다.

**에이전틱 엔지니어링의 장기적인 방향은 human in the loop(HITL), 즉 실행 과정에 반복적으로 개입하는 사람을 제거하는 것**이라고 생각한다.[^3]

지금 당장 모든 사람을 빼자는 뜻은 아니다.

사람이 왜 계속 개입해야 하는지 찾아보자는 뜻이다. 필요한 데이터에 접근할 수 없는지, 실행 권한이 없는지, 결과를 확인할 방법이 없는지 살펴본다. 다른 팀의 도움이 필요한 일도 포함한다. 이런 문제를 하나씩 해결해 에이전트가 스스로 일을 끝낼 수 있게 만드는 것이 목표다.
...
최근 공개된 adaptive workflow는 작업에 따라 절차를 바꾸는 방식이다. 단순한 버그 수정과 새 시스템 개발에 똑같은 절차를 적용하지 않고, 어떤 단계가 필요한지와 얼마나 자세히 다룰지를 고른다. 초기 버전보다 분명 나아진 방향이다.
...
다만 나는 **앞으로 실제 서비스를 개발하는 방식**이 달라질 것이라고 생각한다.
...
결과를 믿을 수 없어 사람이 일일이 읽는다면, 결과가 만족해야 할 조건을 정하고 테스트로 확인한다. 로그와 지표를 남겨 실제로 어떻게 동작했는지도 확인할 수 있게 한다.
...
각 원은 사용자가 처음부터 끝까지 써볼 수 있는 작은 기능 하나다. 화면뿐 아니라 그 뒤의 처리까지 함께 만드는 이런 단위를 vertical slice라고 한다. 원의 크기는 문제에 따라 더 작게도, 조금 크게도 바꿀 수 있다.
...
한 번도 본 적 없는 제품의 요구사항을 회의실에서 미리 쏟아낼 필요가 없다. 직접 써보고 익숙해지면서 다음에 필요한 것을 정할 수 있다. 머릿속으로 제품 전체를 상상하고 판단해야 하는 부담, 즉 인지부하도 낮아진다.
...
그런데 각 단계에서 관련된 사람들이 모여 문제와 배경을 함께 이해하는 과정은 그대로다. AI가 만든 계획과 결과물을 검토하고, 승인한 뒤 다음 단계로 넘어가는 방식도 유지된다.
...
다만 그것은 AI-DLC만의 효과라기보다 Amazon의 팀 문화가 잘 작동한 결과일 수도 있다. 작은 팀이 빠르게 결정하는 Two-Pizza Team 방식과, 개발부터 운영까지 함께 책임지는 DevOps 문화다.[^5]
...
모델이 일하려면 배경 정보와 도구, 접근 권한, 코드를 실행할 환경과 결과를 확인할 방법이 필요하다. 여기서는 이 모두를 하네스라고 부른다.[^6]

에이전트가 사람 없이 더 오래 일할수록 좋은 하네스는 반드시 필요하다.
...
계획을 세우는 Planner, 문제점을 찾는 Critic, 결과를 검토하는 Reviewer, 작업을 되돌아보는 Reflection Agent처럼 역할을 나눠 여러 에이전트를 쓰는 방식이 있다. 사람이 일을 나누는 순서를 미리 정해두기도 한다. 모델이 스스로 문제를 풀기 어려웠던 시기에, 부족한 능력을 이런 절차로 보완하려는 시도였다.
...
과거 모델의 약점을 보완하려고 만든 규칙이 새 모델에게는 오히려 방해가 될 수 있다. 이런 규칙을 걷어내지 못하고 계속 유지하는 문제를 나는 **Harness Debt**, 하네스 부채라고 부르고 싶다.
...
하지만 최신 모델을 쓸 때 사용자가 더 많이 손본다고 결과가 더 좋아지는 것은 아니다.

사용자가 정한 절차 때문에 모델이 더 나은 방법을 선택하지 못할 수도 있기 때문이다. 결제나 금융 업무 중에는 반드시 지켜야 하는 실행 순서와 승인 규칙이 있다. 그런 일에는 정해진 절차가 필요할 수 있지만, 모든 소프트웨어 개발을 같은 방식으로 해야 하는지는 별개의 문제다.

Claude Code와 Codex를 만드는 조직은 모델과 에이전트 제품을 함께 개발한다. 그래서 새 모델이 나올 때마다 어떤 정보와 지침을 제공해야 하는지, 어떤 판단은 모델에게 맡겨도 되는지 함께 확인할 수 있다. 이처럼 모델에게 필요한 정보와 지침을 구성하는 일을 context engineering이라고 한다.
...
OpenAI의 Codex 사례도 모델에게 생각할 순서를 길게 가르치는 데 집중하지 않는다. 코드가 정해진 구조와 규칙을 지키는지 자동으로 검사하고, 테스트로 결과를 확인하는 데 집중한다.[^9]
...
AI-DLC의 adaptive workflow는 모든 작업에 같은 절차를 적용하던 문제를 상당히 줄였다.

그럼에도 단계마다 무엇을 만들고, 언제 승인받고, 언제 함께 검토할지를 도구에 정해두고 있다. 나는 이것이 여전히 모델의 사고방식과 작업 순서를 정해주는 장치에 가깝다고 생각한다.
...
실제 고객의 문제를 고르고, 팀을 이끌고, 서비스를 출시하고 싶다. 그 뒤의 장애와 조직 안에서 부딪히는 문제까지 겪어보고 판단하고 싶다.
...
AI가 코드를 만드는 능력의 개인차를 줄일수록 팀이 함께 일하는 방식이 더 중요해진다. 풀려는 문제와 배경을 공유하고, 결과를 확인하고, 같은 실패를 반복하지 않도록 하네스를 고쳐야 한다. 같은 도구를 사용한 Amazon 팀들의 성과가 크게 갈린 이유도 여기에 있었다.[^5]
...
그런 팀을 찾지 못한다면 AI를 활용하는 작은 회사를 직접 만드는 선택도 고민하고 있다. 작게 만들어 고객 반응을 확인하고 다음 결정을 내리는 Lean Startup 방식을 실험해보고 싶다.[^12] 고객 문제를 고르는 일부터 개발 방식과 구현, 운영까지 직접 책임지며 내 생각이 맞는지 확인할 수 있기 때문이다.

**내 생각이 실제 팀과 고객 문제 앞에서도 통하는지 확인하고 싶다.** 지금은 그 경험을 쌓을 팀을 찾고 있다.
