...
나는 개발자로 커리어를 시작했고, SA(Solutions Architect)가 된 뒤에도 개인 서비스를 직접 개발하고 배포하며 운영해왔다. 그래서 여러 고객의 문제를 넓게 보는 경험보다, 하나의 제품을 오래 소유하며 결정의 결과를 겪는 경험에 더 큰 갈증을 느끼는 편이다.
...
다만 나는 **앞으로 프로덕션 개발이 수렴할 기본 형태**를 다르게 예상한다.
...
잡스는 여러 회사를 넓게 볼 수 있어도 자신의 권고를 구현하고 결과와 실패를 오래 소유하지 않으면 경험이 얇을 수 있다고 말했다. 과일 사진은 많이 봤지만 직접 맛보지는 못했다는 비유다.[^11]

이를 SA 전체에 대한 평가로 가져오고 싶지는 않다. 고객과 오래 일하고, 여러 회사에서 반복되는 패턴과 선택지를 보여주는 것 역시 분명한 전문성이다.

다만 개발자로 시작한 내게는 `권고한 뒤의 운영 결과를 직접 겪는가`라는 질문이 남았다. 개인적으로 20만 줄과 10만 줄 규모의 서비스를 운영하며 시간과 비용, 결과를 소유하고 있지만 공식적인 고객 사례는 아니다.

내가 에이전트 주도 개발이라고 부르던 현상도 Andrej Karpathy가 `Vibe Coding`이라는 이름을 붙인 뒤 훨씬 빠르게 공용어가 됐다. 메시지는 내용뿐 아니라 메신저가 만든 결과에서도 힘을 얻는다.[^12]

SA로서 얻은 넓이는 자산이다. 하지만 지금 필요한 것은 한 팀과 제품을 오래 소유하고, 내 결정의 실패와 운영 결과를 다음 선택에 반영하는 경험이라고 생각한다.

반대로 제품 조직에 속한다고 ownership이 자동으로 생기는 것도 아니다. 주어진 티켓만 처리하고 결과에는 관심 없이 paycheck만 챙기는 사람도 봤다. **Ownership은 직함이 아니라 문제 정의와 운영 결과에 끝까지 참여할 때 생긴다.**

AI가 구현 역량의 개인차를 줄일수록 팀이 문제와 컨텍스트를 공유하고, 결과를 검증하며, 실패를 다음 하네스에 남기는 능력이 더 중요해진다. 같은 도구를 사용한 Amazon 팀들의 성과가 크게 갈린 이유도 여기에 있었다.[^5]

그래서 내가 찾는 것은 특정 직함보다 **결과를 함께 소유하고 실패를 학습으로 남기는 팀**이다.

## 마치며

AI-DLC는 AI를 처음 도입하거나 강한 거버넌스가 필요한 조직에는 현실적인 답이 될 수 있다. 다만 나는 에이전틱 엔지니어링이 사람을 더 잘 배치하기보다, 사람이 필요했던 이유를 시스템에서 제거하는 방향으로 갈 것이라고 믿는다.

SA로서 얻은 넓은 관점은 분명 자산이다. 하지만 개발자였던 내가 다음에 쌓고 싶은 것은 더 많은 논리나 글이 아니라, 실제 팀과 제품의 결과를 오래 소유한 경험이다.

현재 가장 지치는 것은 다른 생각 자체보다, 이를 편하게 논의할 곳이 거의 없다는 점이다. 고객 문제에 도달하기도 전에 HITL 제거와 작은 피드백 루프가 왜 중요한지 설명하고 설득하는 `why`에 시간과 에너지를 대부분 쓴다.

에이전틱 엔지니어링을 직접 경험한 팀이라면 모든 결론이 같지 않아도 된다. 공통 경험을 바탕으로 **고객 문제를 어떻게 풀고 프로세스와 하네스를 어떻게 바꿀 것인지라는 `how`에 더 집중할 수 있기** 때문이다.

그래서 연말까지는 이직하려고 한다. 회사 이름이나 직함보다, 고객 문제를 진지하게 고민하고 운영 결과와 실패를 함께 소유하는 팀을 찾고 싶다. 또는 AI의 발전 방향이 잘 맞는 팀에서 앞선 프랙티스를 배우고 전파하고 싶다.

그런 팀을 찾지 못한다면 AI 기반 Lean Startup을 실험할 작은 회사를 직접 만드는 선택도 고민하고 있다.[^13] 고객 문제부터 컨셉·프로세스·구현과 결과까지 모두 소유하며 내 가설을 검증할 수 있기 때문이다.

현재는 부족한 지점을 정확히 특정하지 못했다. 그래서 아무 곳으로나 옮기기보다, **내게 비어 있는 경험과 함께 일할 팀의 기준을 점검하는 중**이다.
