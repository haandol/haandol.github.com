...
따라서 해당 토큰 들은 AWS SSM 에서 제공하는 파라미터 스토어에 저장하고 코드에서 토큰을 분리해버린다.

AWS SSM 파라미터 스토어는 비연결성으로 제공되는 키 밸류 스토어라고 생각하면 된다. 저장되는 모든 값은 암호화되어 저장된다.
...
서버한대없이 슬랙봇을 설정했다. 해당 봇을 테스트해보자.

## 봇 테스트해보기

1. beebot 에게 DM 으로 `!help` 라고 보내보고 적절히 응답이 오는지 확인한다.
