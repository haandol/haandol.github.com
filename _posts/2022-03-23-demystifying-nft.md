...
나같은 코린이 개발자 추천공부 루트는

* NFT 와 이더리움에 대한 기본적인 유튜브들을 좀 보고 개념을 잡는다.
* 크립토좀비[^1] 로 솔리디티와 익숙해진다.
* hardhat 이나 truffle 튜토리얼을 한다.
* 약간 심화된 튜토리얼[^2]을 해보고 테스트넷에 올려본다.
* 본인만의 컨트랙트 규칙을 가진 토큰을 한두개 만들어보고 테스트넷에 올린다.

(이후에는 다른 프로젝트들 컨트랙트들을 읽어보면 될듯)
...
위의 크립토 키티를 예로 들면 저 각각의 고양이(?) 가 하나의 토큰이다. 크립토 키티에서는 절대 같은 이미지가 존재하지 않는다.

각 토큰이 유니크할 수 있는 이유는 서로 다른 값(여기서는 이미지, 정확히는 이미지를 결정하는 어떠한 값)을 가지기 때문이다.

그리고 그 값을 ERC721 표준에서는 메타데이터라고 부르고 있고, 이 메타데이터는 일반적으로 JSON 데이터를 가리키는 URI (tokenURI) 이다.

ECR1155 는 ERC721 과 거의 같지만, 한 종류(?)의 토큰이 여러개 발행될 수 있다.(토큰 A를 100개 한정으로 발행할 수 있다)

## IPFS

위의 개별고양이 이미지를 일반적인 서버에 저장해두면 해당 서버가 다운되면 내 토큰의 가치도 사라지게 된다.

이것은 특정한 서비스나 회사에 의해 내 이미지가 변경될수도 있고 사라질 수도 있다는 의미이다.

따라서 이 이미지들도 블록체인에 저장해두는 것이 좋다.

하지만 메인넷으로 불리는 이더리움 체인에 이미지나 동영상을 저장하는 것은 엄청난 가스피를 소모하게 될 것이다.

따라서 이것을 블록체인 처럼 분산으로 저장하는 방식이 IPFS[^4] 이다.
...
NFT 에서 많이 쓰이는 chainlink 의 VRF 가 랜덤넘버를 생성해주는 오라클 서비스이다.


## 마치며

그냥 마치면 아쉬우니 웹 3.0[^5]을 보는 개인적인 관점, 그리고 NFT 가 진짜 사기인지? 에 대한 개인적인 생각을 적고 마쳐본다.
...
[^2]: [NFT/ERC-721/Collectible END-TO-END TUTORIAL | Deploy, List on Opensea, Host Metadata on IPFS](https://www.youtube.com/watch?v=p36tXHX1JD8&t=2759)
[^3]: [SmartContract](https://www.youtube.com/watch?v=ZE2HxTmxfrI)
[^4]: [간단하게 설명하는 IPFS](https://www.youtube.com/watch?v=5Uj6uR3fp-U)