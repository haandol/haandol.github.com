...
서비스 뿐 아니라 에이전트용 툴인 [ALPS Writer](https://github.com/haandol/alps-writer-plugins) 나 [PPT Generator](https://github.com/haandol/ppt-generator) 도 동일한 형태로 되어 있다.

두 서비스 프로젝트 모두 Nx 모노레포에 ADR-first 워크플로우를 얹은 같은 골격이라, 주로 EncBird를 예로 들되 필요하면 PixelBank도 함께 언급하겠다.
...
그래서 나는 [ALPS Writer](https://github.com/haandol/alps-writer-mcp) 같은 도구로 PRD부터 쓴다.
...
여러번 반복되는 실수를 어떻게 한 세션안에서 해결한 뒤에, 다음 내용을 입력하자.

`지금 수정한 내용의 원인을 분석해서 재발하지 않도록 AGENTS.md 와 문서들을 업데이트 해줘`
