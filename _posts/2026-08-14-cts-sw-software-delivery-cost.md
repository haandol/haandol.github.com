...
이 글에서는 CTS-SW를 어떻게 정의하고 읽을지 정리한다. 인지부하와 리뷰 대기열을 개인·팀에서 어떻게 줄일지는 별도 글에서 다룬다.[^5]
...
리뷰 대기열이 늘었다면 다음 질문은 ‘더 빨리 승인할 수 있는가’에서 끝나지 않는다. 왜 기다리는지 진단하고, 사람이 판단할 범위를 줄이거나 검증을 개선할 작업을 정해야 한다. 그 구체적인 대응은 개인·팀 단위의 인지부하 글에 이어서 정리했다.[^5]
...
[^5]: [AI 코드는 빨라졌는데 왜 일은 밀릴까 — 개인의 인지부하와 팀의 미완료 작업 줄이기](/2026/08/18/ai-coding-review-cognitive-load.html) — 작은 이해 단위, 계약·검증, 진행 중 작업 제한과 병목 대응을 다룬다.
