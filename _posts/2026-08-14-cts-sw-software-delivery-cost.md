...
같은 인원이 같은 한 주 동안 20회를 안정적으로 전달했다면 `0.4 개발자-주`로 내려간다. 같은 소프트웨어 단위를 전달하는 데 사용한 엔지니어링 여력이 줄었다고 볼 수 있다.
...
Amazon은 개발 쪽과 배포 쪽의 산출량을 나누어 본다. 개발 속도는 개발자당 한 주에 병합된 코드 검토 건수, 배포 속도는 개발자당 한 주에 이뤄진 운영 배포 횟수로 측정한다.[^2] 여기서 속도는 검토 한 건의 소요 시간이 아니라 일정 기간의 처리량이다.

두 지표를 나눠 보면 코드 변경이 늘어도 실제 배포로 이어지는지 확인할 수 있다.
...
이 주장을 보여주는 사례가 Amazon의 50개 팀 Frontier Development pilot이다.[^11]

기존 코드베이스를 운영하는 비슷한 구성의 팀들이 거의 같은 AI 도구를 사용했는데도, 절반은 운영 환경으로 변경을 전달하는 속도가 3배 미만으로 개선됐다. 다른 절반은 중앙값이 4.5배였고 일부는 10배를 넘었다.

발표자가 주목한 차이는 일하는 방식이었다. 성과가 크게 오른 팀들은 컨텍스트, 도구, 테스트와 의도 문서를 함께 정비해서 Agent가 사람의 대답을 기다리지 않고 스스로 검증할 수 있게 만들었다.

팀 하네스가 어떤 차이를 만드는지 살펴볼 사례지만, 관찰만으로 각 요소의 효과를 따로 확정할 수는 없다.

다만 배포 속도가 빨라졌다는 사실만으로 CTS-SW가 낮아졌다고 결론 내릴 수는 없다. 추가한 하네스를 만들고 운영하는 비용, 리뷰와 장애 대응, 전달 품질까지 같은 범위에서 확인해야 한다.
...
소프트웨어 개발·운영 성과를 연구하는 DORA의 2025년 분석이 AI를 기존 조직 역량의 증폭기로 설명하는 것도 이 흐름과 비슷하다. 자동 테스트, 버전 관리와 빠른 피드백이 약한 조직에서는 늘어난 변경량이 불안정성으로 이어질 수 있다.[^8]
...
제품 담당자는 고객에게 도달한 단위를 정의하고, 서비스의 신뢰성과 운영을 맡는 SRE는 품질과 운영 비용을 확인한다. 엔지니어링 매니저는 팀 구성 변화와 지표의 사용 범위를 관리한다. 시니어 엔지니어는 숫자에서 실제 PR, 배포와 장애까지 다시 추적할 수 있게 연결하는 역할을 맡을 수 있다.

처음부터 Amazon의 분석 모델을 그대로 재현하는 것보다, 한 팀에서 같은 정의로 전후 변화를 설명할 수 있게 만드는 것이 먼저라고 생각한다.

## 마치며

지금은 한 팀에서 고객에게 도달한 단위와 품질 기준을 정하고, 가장 큰 병목 하나를 고쳐보는 데 CTS-SW를 쓰는 편이 낫다고 생각한다.

최근 기준선과 비교하되, 숫자가 낮아졌다면 비용이 리뷰나 장애 대응으로 빠져나간 것은 아닌지 함께 확인한다. 소프트웨어 단위와 비용 범위를 같은 의미로 유지해야 이 비교도 가능하다.

이때 하네스를 고칠 시간까지 팀의 정식 작업으로 배정하고 싶다. 개인이 알아낸 요령을 공유 계약과 도구, 테스트로 옮겨야 다음 작업과 다른 구성원에게도 개선이 남기 때문이다.
...
[^11]: [같은 AI 도구를 썼는데 왜 어떤 팀은 10배까지 빨라졌을까](/2026/08/31/frontier-development-habits.html) — Amazon의 50개 팀 pilot과 Frontier Development의 다섯 습관을 팀 하네스 관점에서 정리한다.
