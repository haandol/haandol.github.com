...
- 하네스 0층은 추상화 레벨이 섞이는 순간 무너진다.
- PRD는 ALPS로 고정하고, 결정은 ADR로 떼어낸다.
- 태스크 문서는 코딩 에이전트가 흡수해 휘발되고 있다.

## 시작하며

이전 글에서 하네스를 한 겹씩 쌓아온 과정을 이야기하면서, PRD와 ADR을 **0층**으로 꼽았다.[^1] 그 위에 컨텍스트, 도구, 가드레일, 서브에이전트가 쌓인다는 이야기였다.

그런데 그 글에서 0층은 두 문단으로 지나갔다. 위층들은 어떻게 쌓는지 자세히 썼는데, 정작 기준점이 되는 층은 "PRD와 ADR을 둔다"로 끝냈다.

위층 하네스는 프로젝트가 커져도 대체로 잘 버틴다. 훅과 lint는 코드베이스가 10배가 되어도 그냥 돌아간다.

**무너지는 건 0층이다.** 사용자가 붙어 있고 코드가 매일 바뀌는 시스템에서, 문서는 조용히 거짓이 되고 아무도 안 보게 된다. 기준점이 사라지면 그 위에 쌓은 층들은 각자 다른 방향을 가리킨다.

나도 거기서 제일 크게 실패해봤다. EncBird에 게임 모드 하나를 붙이면서 621줄짜리 PRD를 썼는데, 그 문서는 커밋 3개를 남기고 죽었다. 코드는 살아남았고 문서만 죽었다.

원인은 분량이나 성실함이 아니라 **추상화 레벨**이었다고 지금은 생각한다. 새로운 이야기도 아니다. 클린 아키텍처와 DDD가 도메인 로직을 지키려고 그렇게 애쓰는 이유와 정확히 같은 문제다.

이 글은 그 0층을 자세히 다룬다. 앞은 [ALPS Writer](https://github.com/haandol/alps-writer-plugins)[^2]로 PRD를 왜 쓰고 어떻게 쓰는지, 뒤는 `adr-writer`로 그 아래 경계를 어떻게 지키는지다.
...
그래서 의존 방향은 한 방향이어야 한다. 느린 층이 빠른 층을 참조하면, 매일 바뀌는 것 때문에 몇 달에 한 번 바뀌어야 할 문서를 매일 고쳐야 한다.

**클린 아키텍처가 도메인을 프레임워크로부터 지키는 이유와 같다.** 도메인 로직이 ORM 클래스를 직접 참조하면 DB를 바꿀 때 비즈니스 규칙이 흔들린다. 그래서 화살표를 안쪽으로만 향하게 만든다.

문서도 똑같다. **PRD가 코드를 참조하면, 리팩터링 한 번에 PRD가 거짓이 된다.**
...
RFTCR 글에서 C4 모델을 예로 들며 "단계별 입출력의 추상화 수준을 명확히 정의해야 한다"고 썼는데[^3], 그때는 그게 프로세스 표준화 얘기라고 생각했다. 지금은 **문서의 수명을 결정하는 문제**라고 본다.
...
`/alps-init`을 치면 대화가 시작된다. [성경길](https://bible-atlas.encbird.com)[^4]이라는 사이드 프로젝트를 이걸로 시작했는데, 실제 흐름은 이랬다.
...
RFTCR에서 나는 다섯 단계를 제안했다. Requirement → Feature → Task → Code → Reflect.[^3] 그중 Task 단계에 대해 "TaskMaster 같은 도구로 세부 작업을 뽑고 의존 순서를 관리하라"고 썼다.
...
지금 남겨야 하는 건 태스크가 아니라 **태스크를 만들어낼 수 있는 상위 정보**다.
...
동시에 층 하나는 사라지는 중이다. 태스크는 이제 파일이 아니라 에이전트 세션 안에서만 존재한다. 남겨야 할 건 태스크가 아니라 태스크를 다시 뽑아낼 수 있는 결정이다.
...
[^2]: [ALPS Writer Plugins](https://github.com/haandol/alps-writer-plugins) — 마켓플레이스, 사용 가이드, 의존성 모델 문서. ADR 형식 자체의 원전은 [Documenting Architecture Decisions](https://cognitect.com/blog/2011/11/15/documenting-architecture-decisions).

[^3]: [RFTCR - 에이전트 주도 소프트웨어 개발을 위한 새로운 SDLC 프레임워크](/2025/05/11/rftcr-framework-for-agentic-dev.html) — 단계별 입출력의 추상화 수준을 C4 모델처럼 고정해야 한다는 논지. 이 글의 5절은 그중 Task 단계에 대한 정정이다.

[^4]: [성경길 Bible Atlas](https://bible-atlas.encbird.com) — ALPS 문서를 먼저 쓰고 시작한 사이드 프로젝트.
