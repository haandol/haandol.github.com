...
이 원칙을 실제 프로젝트에 적용하려고 만든 도구가 [ALPS Writer](https://github.com/haandol/alps-writer-plugins)[^3]와 `adr-writer`다. 오늘은 **이 도구가 어떤 관점에서 나왔고, 그 관점이 포맷과 워크플로우에 어떻게 반영됐는지** 이야기해보려고 한다. PRD와 ADR을 하네스의 0층으로 두는 이유부터 실제 사용법까지 차례로 살펴본다.[^1]
...
```mermaid
flowchart TD
    PRD["<b>PRD</b> — 무엇을 왜 만드나<br/><i>수명: 제품 가설이 유효한 동안</i>"]
    ADR["<b>ADR</b> — 왜 이 대안을 택했나<br/><i>수명: 결정의 전제가 유효한 동안</i>"]
    CODE["<b>코드</b> — 지금 어떻게 동작하나<br/><i>수명: 다음 변경까지</i>"]

    PRD -->|"목표 · 범위로 제약"| ADR
    ADR -->|"결정 · 불변식으로 제약"| CODE
    CODE -->|"실행 · 테스트"| RUN["운영에서 관찰"]
    RUN -.->|"제품 가설이 틀렸다는 증거"| PRD
    RUN -.->|"결정의 전제가 틀렸다는 증거"| ADR
```
...
달력의 숫자보다 중요한 건 **무엇이 바뀔 때 함께 고쳐야 하는가**다. 내가 구분하는 세 층의 수명은 다음과 같다.

| 층 | 담는 질문 | 보통의 수명 | 갱신 신호 | 넣지 않는 것 |
| --- | --- | --- | --- | --- |
| PRD | 무엇을 왜 만들고, 성공은 무엇인가 | 제품 가설·범위가 유효한 동안(대개 몇 달) | 타깃 문제, 성공 기준, 범위가 바뀜 | 파일 경로, 함수, 구현 순서 |
| ADR | 왜 이 대안을 택했고 무엇을 지켜야 하는가 | 결정의 전제·트레이드오프가 유효한 동안(대개 몇 주~몇 달) | 기존 전제가 깨지거나 대안을 다시 선택함 | 코드로 바로 알 수 있는 구조·상수 |
| 코드 | 지금 시스템은 어떻게 동작하는가 | 다음 구현 변경까지(몇 시간~며칠) | 기능 추가, 버그 수정, 리팩터링 | PRD·ADR 번호의 역참조 |

여기서 긴 수명은 문서를 고정한다는 뜻이 아니다. 운영에서 제품 가설이나 결정의 전제가 틀렸다는 증거가 나오면 해당 층을 갱신한다. 반대로 함수명이나 파일 배치만 달라졌다면 PRD와 ADR은 그대로 살아 있어야 한다.
...
## 2. 이 관점이 ALPS Writer가 되기까지

그럼 PRD를 어떻게 자기 층에 머물게 하면서 에이전트가 읽기 좋은 source of truth로 만들 수 있을까. ALPS Writer는 여기에 두 가지로 답한다. 포맷을 고정하는 것, 그리고 작성 책임을 뒤집는 것이다.
...
ALPS Writer와 `adr-writer`는 이 관점을 도구로 옮긴 결과다. 전자는 요구사항의 형식과 완결성을 지키고, 후자는 요구사항과 코드 사이의 결정을 분리한다. 둘 다 문서를 더 많이 만들기보다 **오래 남길 컨텍스트만 정확한 추상화 수준에 남기는 것**을 목표로 한다.
