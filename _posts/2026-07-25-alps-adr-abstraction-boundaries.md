...
Office Raid PRD를 열면 세계관도 나오고 스토어 변수도 나온다. 그래서 어떤 질문을 들고 가도 621줄 전체를 훑어야 했다. 반대로 ADR은 142줄 안에서 그 기능의 판정 기준만 다루기 때문에 필요할 때 그것만 열면 됐다.
...
C4 모델에는 층을 섞었는지 검사해주는 도구가 없다. 사람이 다이어그램을 보고 판단한다. 문서 레이어에서는 그 판단 중 일부라도 스크립트로 내려보내려고 했다. 한 방향 의존을 원칙이 아니라 **깨질 수 있는 검사**로 만들어두는 것. 이게 사람의 성실함에 의존하지 않는 유일한 방법이라고 생각한다.
