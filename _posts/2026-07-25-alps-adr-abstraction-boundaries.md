...
- PRD는 제품 전체의 가설을, ADR은 기능 하나의 요구사항을 담는다.
- ADR의 목표는 같은 코드가 아니라 같은 요구사항을 지키는 코드다.
- PRD는 거푸집이라 ADR로 다 넘긴 뒤에는 지운다.
...
이 원칙을 실제 프로젝트에 적용하려고 만든 도구가 [ALPS Writer](https://github.com/haandol/alps-writer-plugins)[^3]와 `adr-writer`다. 오늘은 **이 도구가 어떤 관점에서 나왔고, 그 관점이 포맷과 워크플로우에 어떻게 반영됐는지** 이야기해보려고 한다. PRD와 ADR을 서로 다른 레이어로 갈라두는 이유부터 실제 사용법까지 차례로 살펴본다.[^1]
...
레이어마다 **변하는 속도가 다르다.** 이게 레이어를 나누는 유일한 이유다.

제품 전체의 요구사항은 몇 달에 한 번 바뀌고, 기능 하나의 요구사항은 몇 주에 한 번 바뀌고, 코드는 매일 바뀐다.
...
그림으로 겹쳐 놓으면 이렇게 된다. 안쪽이 느리게 바뀌고 바깥이 빨리 바뀐다는 점, 그리고 화살표가 한 방향뿐이라는 점이 클린 아키텍처와 같다.

![ALPS·ADR·코드·코딩 에이전트의 의존 관계](/assets/img/2026/0725/dependency-rings.svg)

여기서 클린 아키텍처와 다른 게 하나 있다. 클린 아키텍처에서는 바깥 레이어가 안쪽의 인터페이스를 **코드로** 참조한다. 컴파일러가 그 화살표를 검사해준다.

문서에는 그런 컴파일러가 없다. 그래서 화살표를 아예 긋지 않는 쪽을 택했다. **코드는 ADR을 모르고, ADR은 PRD의 파일명을 모른다.**

대신 그 자리를 코딩 에이전트가 맡는다. 요구사항과 코드를 함께 읽고 둘 사이의 매핑을 그때그때 복원한다. 저장해둔 매핑이 없으니 stale해질 것도 없다.
...
이 문장들은 **코드를 아무리 리팩터링해도 사실과 어긋나지 않는다.** 그래서 두 달 뒤 후속 결정(무료 플레이, 게임 종료 후 표현 추천)을 추가할 때도 그 ADR 옆에 새 ADR을 놓는 것으로 끝났다. PRD는 손대지 않았다.
...
ALPS Writer는 여기서 출발했다. PRD 작성을 편하게 해주는 생성기가 필요해서가 아니라, **요구사항을 코드의 변화로부터 지키면서 에이전트가 계속 사용할 수 있는 컨텍스트로 만들기 위해서**였다.
...
Office Raid PRD가 어긴 게 정확히 이 규칙이다. `coffee = ref(3)`은 DB를 바꾸면 사실과 어긋날 수도 있는 문장이었다.
...
그리고 왼쪽은 구현이 끝난 뒤 **검사할 수 있는 문장**이다. 세 번째 항목을 예로 들면, SRS due date로 표현을 조회하는 방식은 "많이 공부할수록 플레이 불가" 역설을 일으켜서 이 요구사항을 위반한다. 그래서 기각됐다. 코드가 어떻게 생겼든 이 판정은 그대로 작동한다.
...
flowchart TD
    ...
    F2A --> DROP["ALPS 문서 제거<br/><i>거푸집을 떼어낸다</i>"]
...
| --------- | ------- | --------- | --------------------- |
| 성경길    | 2026-07 | 없음      | 15 카테고리 / 19 ADR  |
| PixelBank | 2025-09 | 없음      | 10 카테고리 / 57 ADR  |
| EncBird   | 2023-02 | 없음      | 20 카테고리 / 132 ADR |

세 프로젝트 다 PRD부터 썼는데, **세 곳 다 지금은 ALPS 문서가 없다.** `/feature-to-adr`로 기능을 다 넘긴 뒤 지웠다.

처음에는 이게 아까웠다. 공들여 쓴 9개 섹션인데.
...
즉 PRD는 역할이 남지 않았는데 파일만 남은 상태가 된다. 그 상태로 두면 import 시점의 스냅샷으로 굳어서, 읽으면 오히려 현재와 어긋난 정보를 준다.

**PRD는 거푸집이었다.** 요구사항을 부어 굳히는 동안은 반드시 필요하고, 굳은 다음에는 떼어내는 것이다. 떼어내지 않으면 콘크리트가 아니라 거푸집을 보게 된다.

이게 앞서 말한 "저장소에 남는 텍스트는 모두 최신이어야 한다"는 원칙의 결론이기도 하다. 최신으로 유지할 수 없는 문서라면 최신으로 유지하는 대신 지우는 선택지가 있다.

Office Raid PRD의 실패도 다시 보면 같은 얘기다. 그건 새 제품의 PRD가 아니라 **3년 된 시스템의 한 기능에 대한 PRD**였다. 거푸집이 필요한 자리가 아니었는데 거푸집을 세우고, 떼어내지도 않았다.
...
프로덕션에서 컨텍스트를 유지한다는 건 문서를 많이 쓰는 게 아니었다. **각 문서가 자기 레이어에 머무르게 만드는 일**이었다.

저장소에 남기는 텍스트는 모두 에이전트가 읽을 일급 컨텍스트로 취급하고 최신으로 유지해야 한다. 그 비용을 감당할 가치가 없는 것은 애초에 남기지 않거나, 역할이 끝나면 지우는 편이 낫다. 태스크와 요약이 전자이고, 기능을 다 넘긴 PRD가 후자다.

ALPS Writer와 `adr-writer`는 이 관점을 도구로 옮긴 결과다. 전자는 제품 전체의 가설을 빠뜨림 없이 세우게 하고, 후자는 그것을 기능별 요구사항으로 굳혀 구현의 판정 기준으로 만든다. 둘 다 문서를 더 많이 만들기보다 **오래 남길 컨텍스트만 정확한 추상화 수준에 남기는 것**을 목표로 한다.

여기서 제일 중요한 게 ADR의 목표를 오해하지 않는 것이다. **같은 코드를 다시 만들려는 문서가 아니라, 같은 요구사항을 지키는 코드를 다시 만들 수 있게 하는 문서다.** 구현의 세부는 개발하는 과정에 맡기고, 그 결과가 요구사항을 준수하는지만 판정한다.
...
이게 컨텍스트 위에 얹는 다른 자동화들과 똑같은 패턴이라는 점이 흥미롭다. 사람이 매번 메우던 빈틈을 찾아서, 그 자리에 파일 한 겹을 덧대는 것.[^1] 요구사항 문서도 결국 같은 방식으로 유지된다.

동시에 레이어 하나는 사라지는 중이다. 태스크와 구현 요약은 이제 파일이 아니라 에이전트 세션 안에서만 존재한다. 남겨야 할 건 중간 산출물이 아니라 그것을 다시 뽑아낼 수 있는 요구사항이다.

돌아보면 내 저장소에 결국 남은 것도 ADR뿐이었다. 컴파일러에 넣을 입력만 남고, 출력을 설명하던 문서는 사라졌다.

이미 돌아가는 시스템에 이 레이어를 세우고 싶다면, PRD를 소급 작성하지 말고 다음에 손댈 기능 하나에 `/adr-new`부터 써보시길 권한다.
