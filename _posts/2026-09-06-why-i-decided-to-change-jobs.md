...
지난 1년 동안은 내가 믿지 않는 내용을 앞장서서 전달해야 하는 상황을 꽤 잘 피해왔다. 다른 주제를 고르거나, 내가 동의하는 일부만 설명하거나, 아주 가까운 사람들에게만 제한적으로 반대 의견을 말하는 식이었다.
...
문제는 내가 동의하지 않는 내용을 전달하는 일에 적극적으로 나설 마음이 생기지 않는다는 것이다.

방향이 틀렸다고 생각하면서 직급과 안정성 때문에 남아 있고, 뒤에서는 의심하면서 앞에서는 동료들에게 열심히 하자고 말하는 모습은 팀에도 나에게도 정직하지 않다.

**내가 믿지 않는 방향을 전달하지 않는 것과, 시니어로서 해야 할 역할을 하지 않는 것은 결국 같은 일이었다.**

그래서 이 글은 AI-DLC가 객관적으로 틀렸음을 증명하는 글이 아니다.

AI-DLC를 보면서 내가 왜 현재 역할과 충돌한다고 느꼈는지, 그리고 왜 설명과 지원에 머무르지 않고 결과까지 직접 소유해야 한다고 생각했는지를 정리한 글이다.
...
내가 AI-DLC에 동의하기 어려운 지점은 세 축이다.

AI-DLC가 향하는 **큰 방향**, 실제 업무를 진행하는 **실행 프랙티스**, 그리고 그 프랙티스를 workflow와 하네스로 옮긴 **구현 방식**이다.

## 1. 큰 방향에 동의할 수 없다 — HITL을 남기는 접근
...
내가 반대하는 것은 그 유용성이 아니다.

**이 구조를 앞으로 프로덕션 개발이 수렴할 기본 형태로 보는 것**에 반대한다.
...
나는 이 부분에서 AI-DLC와 같은 미래를 보고 있지 않다.

## 2. 실행 프랙티스에 동의할 수 없다 — 큰 원을 더 빨리 도는 방식
...
내가 경험한 AI-DLC는 큰 원을 작은 원으로 바꾸기보다, 기존의 큰 원을 AI로 더 빨리 돌리는 접근에 가까웠다.
...
## 3. 구현 방식에 동의할 수 없다 — 모델의 판단을 덮는 하네스

AI-DLC를 보며 가장 크게 충돌한 세 번째 지점은 하네스다.
...
이때 하네스가 항상 `Research → Plan → Task 생성 → Implement → Review → Reflection` 순서를 강제하면, 하네스가 모델을 돕는 것이 아니라 모델 대신 사고하기 시작한다.

과거 모델의 약점을 보완하려고 만든 제약이 다음 모델에서도 남아 더 나은 판단을 막는 현상을 나는 **Harness Debt**라고 부르고 싶다.
...
사용자가 모델보다 못한 workflow를 정교하게 만들어 강제할 수도 있기 때문이다. 결제나 금융의 일부처럼 실행 순서와 승인 규칙이 업무 계약인 영역에는 이런 구조가 필요할 수 있지만, 모든 소프트웨어 개발의 기본값으로 삼아야 하는지는 별개의 문제다.
...
현장에서 비슷한 이야기를 접할 때마다 내가 완전히 잘못 보고 있는지, 내 개인 프로젝트의 경험을 일반화하고 있는지 스스로 의심하게 된다.

가끔은 내가 스스로를 가스라이팅하는 건지, 반대로 당하고 있는 건지 헷갈린다.

이 의문은 지금 자리에서는 풀기 어렵다고 생각한다.

나는 AI-DLC를 설명하고 도입을 돕는 쪽에 가깝지, 한 제품과 팀을 몇 년 동안 운영하며 그 방법의 결과를 끝까지 책임지는 사람이 아니기 때문이다.

방법론이 맞는지 틀리는지를 프레젠테이션과 워크숍의 반응으로 판단하고 싶지 않다.

실제 고객의 문제를 고르고, 팀을 이끌고, 프로덕션에 배포하고, 장애와 조직의 반작용까지 겪은 뒤에 판단하고 싶다.

그래서 이 문제는 내 가설이 맞다는 논리를 더 쌓는 것보다, **내가 틀릴 수 있는 환경에서 결과를 직접 소유해봐야 확인할 수 있다**고 생각한다.
...
시니어로서 내가 전달해야 하는 방향과 반대되는 미래를 믿으면서 일하는 것은 오래 지속할 수 없다.

SA로서 얻은 넓은 관점은 분명 내 자산이다.

하지만 개발자였던 내가 다음에 쌓고 싶은 것은 더 정교한 논리나 더 많은 글이 아니라, 실제 팀과 제품의 결과를 오래 소유한 경험이라는 결론에 도달했다.

AI-DLC에 반대한다고 회사 안에서 공개적으로 크게 말하기는 어렵다. 이 방법을 만들고 열심히 전달하는 동료들이 있고, 누군가에게는 실제로 도움이 되고 있기 때문이다.

그렇다고 안정적이라는 이유로 내가 믿지 않는 방향을 외면한 채 계속 머물고 싶지도 않다.
...
다음 자리에서는 내가 믿는 방향을 더 가까이에서 배우거나, 실제 고객의 문제에 끝까지 책임지는 경험 중 적어도 하나를 좋은 팀과 함께 분명히 얻고 싶다.
