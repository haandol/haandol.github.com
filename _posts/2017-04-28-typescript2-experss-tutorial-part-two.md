...
개인적으로 앵귤러 공부할 때 히어로 예제가 굉장히 맘에 들었다.
우리는 express 를 이용하여 원피스(애니) 등장인물들의 정보를 조회할 수 있는 간단한 REST API를 만들어보자.
...
구조라고 해봤자 npm, tsc 각각 init 한다음에 src 폴더 만드는거 밖에 없다.

서비스를 처음 구성할 때 다양한 순서가 있을 수 있지만
개인적으로 아래와 같은 순서를 선호하므로 해당 순서대로 진행하겠다.
...
원래는 테스트를 작성하는게 맞지만 글이 복잡해~~길어~~지니깐, 본 글에서는 테스트를 작성하는 부분은 제외하겠다.
...
1. express 를 임포팅 한다. typescript 의 임포팅은 `import * as 이름 from` 방식과 `import { 모듈명 } from`  방식이 있다. 뭘써도 상관없고 코드에 맞춰서 알아서 선택해서 쓰면 된다.
...
뭔가 블로그 처음 목적이 퇴색되어 가는 것 같다...~~먹고 사는게 힘들어서~~
