...
이후 Kiro의 Frontier Engineering과 매니저용 가이드를 읽으며, 이 단계 안에서 팀이 실제로 무엇을 준비하고 운영해야 하는지 보완했다. 특히 한 팀의 작은 업무부터 시작하고, 검증한 방법을 조직에 공유하면서 계속 관리한다는 제안이 참고가 됐다.[^6]
...
이 여유를 확보하는 것은 관리자의 일이기도 하다. 테스트와 도구를 정비하라고 하면서 이전과 같은 기능 출시량을 요구하면, 팀은 당장 평가받는 기능 구현부터 하게 된다.

나는 첫 업무를 정할 때 하네스에 쓸 시간과 예산, 결과를 다시 볼 시점을 함께 잡는 편이 낫다고 생각한다. 그때까지 어떤 도구와 평가 사례가 남았고 반복 개입이 어떻게 달라졌는지 확인하면, 학습에 투자하면서도 효과 없는 실험을 계속 끌고 가지 않을 수 있다.
...
처음에는 결과와 책임자를 분명히 정할 수 있는 한 팀의 작은 업무를 고르는 편이 낫다. 여러 시스템에 걸친 업무라도 처음 검증할 범위와 완료 조건은 좁힐 수 있다.
...
이 업무를 구현하는 작업도 Agent가 실행할 수 있는 형태로 준비해야 한다. 가령 첫 범위를 배송 전 주문의 취소 기능으로 정했다면, 취소할 수 있는 상태, 중복 요청의 처리 방식, 완료 후 보여줄 결과와 확인할 테스트를 작업에 함께 적는다. 기존 서비스라면 현재 동작부터 확인하고, 유지할 동작과 바꿀 동작을 구분한다.

설계 결정과 실행·테스트 방법은 저장소에 남겨 다음 세션에서도 찾을 수 있게 한다. **동료에게 물어야만 알 수 있는 정보를 줄이는 것**도 업무 연결의 일부다.

권한은 문서에 금지 사항을 적는 것만으로 끝내지 않는다. Kiro의 개발자 가이드도 파일, 도구, 네트워크와 인증 정보 접근을 실제로 제한하라고 권한다.[^7] 위 예시라면 개발·테스트 환경에서 취소 흐름을 검증할 권한과 실제 고객에게 환불할 권한을 구분해야 한다.

이 단계의 질문은 성과보다 준비 상태에 가깝다.

- 업무의 시작과 완료 조건이 분명한가
- 필요한 데이터와 도구에 접근할 수 있는가
- 기존 동작과 완료 기준을 확인할 문서·테스트가 있는가
- 권한과 책임 경계가 연결되어 있는가
...
실패를 보려면 Agent가 직접 검증할 수 있어야 한다. 사람이 테스트를 실행하고 오류를 복사해줘야 한다면 매번 사람을 기다리게 된다. Kiro도 로컬 테스트와 브라우저 검증, 외부 서비스를 대신하는 테스트용 도구를 Agent가 사용할 수 있게 하라고 권한다.[^8]

주문 취소 기능이라면 테스트 환경에서 정상 취소와 중복 요청을 실행하고, 실패 내용을 읽어 수정한 뒤 같은 테스트를 다시 돌릴 수 있게 만든다. 다만 테스트 환경의 성공만으로 실제 결제 시스템과의 연동까지 확인됐다고 보지는 않는다. 어떤 환경에서 무엇을 검증했는지 결과에 남겨야 한다.

{% raw %}
```mermaid
flowchart LR
    T["준비된 작업<br/>목적 · 제약 · 완료 기준"] --> A["Agent 구현"]
    A --> V{"자동 검증"}
    V -->|실패| A
    V -->|통과| R["사람이 결과·근거 검토"]
    A -->|새 판단 필요| H["사람이 예외 판단"]
    R --> F["반복 지적의 원인 확인"]
    H --> F
    F --> U["문서 · 도구 · 테스트 개선"]
    U --> T
...
관리자는 그동안 다음에 맡길 작업과 검토할 시간을 함께 준비해야 한다. 구현할 목적이나 선행 결정이 빠진 작업은 먼저 정리하고, 검토가 밀리면 동시에 실행하는 작업 수를 줄인다. Agent의 실행 시간을 늘리는 것만 목표로 두면 팀에 미완료 작업이 쌓일 수 있다.
...
다른 팀에 적용할 때는 기존 팀이 쓰던 규칙 전체를 복사하기보다 공통으로 지킬 규칙과 업무별 규칙을 나눈다. 예를 들어 인증 정보가 코드에 들어가지 않게 검사하는 방법은 공유할 수 있지만, 주문 취소 정책을 정산 업무에 그대로 적용할 수는 없다. 새 업무에서도 품질과 권한 조건을 확인한 뒤 적용 범위를 넓힌다.

공유 자산을 관리하는 책임도 남겨야 한다. 나는 먼저 도입한 팀이나 지원팀이 공통 도구와 사례를 관리하고, 각 도메인팀은 자기 업무의 정책과 평가를 관리하는 방식이 적절하다고 생각한다. 다른 팀의 피드백을 받아 수정할 담당자와 시간이 없으면 처음 배포한 규칙만 남기 쉽다.
...
Kiro도 새 모델이 나오면 이전 모델의 약점을 보완하려고 만든 규칙과 도구가 여전히 필요한지 다시 보라고 제안한다.[^9] 실패 사례를 보존한 채 불필요해진 규칙을 걷어내고 다시 검증하는 작업까지 Scale의 운영비에 포함해야 한다.
...
나는 한 팀의 작은 업무에서 투자 기간과 확인할 결과를 먼저 정하는 편이 낫다고 생각한다. 그 팀이 반복되는 문제를 직접 고칠 수 있게 된 뒤 다른 업무로 넓히고, 공유한 도구를 관리할 시간도 계속 확보해야 한다.

3S는 **지금 이 워크로드에 무엇을 투자하고 무엇을 확인할지 구분하는 렌즈**에 가깝다. 단계 이름이 실제 업무 배분과 평가를 바꿀 때 도입 방법으로 쓸 수 있다.
