...
다이어그램의 유일한(?) 문제점은 같은 시스템을 그리더라도 주제에 따라 그림이 달라진다는 것이다.
...
개인적으로 생각하는 C4Model 장점은 디자인패턴의 그것과 비슷한데, 커뮤니케이션 비용을 줄여준다는 측면에서 그렇다.

화이트보딩을 하거나 회의를 할 때, 다이어그램의 이름만 봐도 추상화 정도를 직관적으로 예측할 수 있으며, 다이어그램을 요청할 때 필요한 만큼의 정보를 담고 있는 다이어그램을 요청할 수 있기 때문이다. (e.g. 관리자 페르소나 대상으로 주문관리 서비스 컴포넌트 다이어그램 그려둔거 있나요?)
...
이러한 복잡한 세부사항들을 한 다이어그램에 담으면 다이어그램을 읽을때 목적이 모호해진다. 가능하다면 한 다이어그램은 하나의 목적을 가지게끔 그리고 여러개의 다이어그램을 그리는 것을 추천한다.
