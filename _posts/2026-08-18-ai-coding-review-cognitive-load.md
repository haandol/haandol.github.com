...
앞의 방식으로 검토 범위를 줄였더라도, 매번 같은 조건을 사람이 확인한다면 그만큼의 부하는 계속 남는다.
...
장기적으로는 이렇게 사람의 반복 판단을 하네스로 옮겨, 정상 경로에서 사람이 매번 개입하는 HITL을 제거하는 방향으로 가야 한다고 생각한다. 사람이 다음 변경을 위해 동작을 이해하는 일과, 매번 같은 조건을 승인하는 일은 구분할 필요가 있다.
