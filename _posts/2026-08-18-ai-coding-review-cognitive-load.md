...
tags: ai agent cognitive-load developer-experience code-review
publish: true
---
...
- 사람의 이해 속도가 전체 개발 속도를 제한한다.
- 계약 기반 리뷰와 작은 변경이 이해할 범위를 줄인다.
...
이 글을 읽으면서 요즘 코딩 에이전트를 쓸 때 느끼는 피로를 `Cognitive Load`로 설명할 수 있겠다고 생각했다.
...
처음에는 AI가 구현에 필요한 인지부하를 줄였다고 생각했다.

그런데 실제로는 부하가 사라진 것이 아니라 **구현 과정에서 리뷰 시점으로 이동한 것**에 가까웠다. 이 글에서는 그 이동이 왜 생기고, 사람이 한 번에 이해해야 할 범위를 어떻게 줄일 수 있을지 생각해본다.
...
그래서 리뷰에서 사람이 맡을 일을 구현 전체의 재구성보다 의도와 경계를 확인하는 쪽으로 옮길 필요가 있다고 생각한다.
...
그래도 인지부하를 계속 줄이는 방향을 기준으로 보면 계약 기반 리뷰 쪽이 더 오래 유효할 것 같다.
...
개발하면서 발견한 요구사항과 경계를 계속 반영할 수 있고, 구현이 바뀌어도 같은 계약을 다시 사용할 수 있다. 계약을 검증하는 테스트와 가드레일이 늘어날수록 사람이 코드에서 다시 추론해야 할 내용도 줄어든다.
...
flowchart LR
    ...
    A --> E{"증거만으로<br/>판단 가능한가?"}
    E -->|Yes| D["승인<br/>코드 전체는 읽지 않음"]
    E -->|No| X["예외 확인<br/>질문 · 코드 리뷰"]
    X --> C
```
{% endraw %}

내가 원하는 형태는 사람이 코드 생성 과정을 계속 따라가며 머릿속에 복원하는 것이 아니다.

사람은 무엇이 참이어야 하는지 정하고, Agent가 그것을 확인하지 못했거나 기존 계약 밖의 결정을 발견했을 때만 코드로 내려간다.

직접 읽은 코드만 신뢰할 수 있다는 전제를 계속 유지하면 AI가 빨라질수록 사람이 병목이 된다. 그렇다고 테스트만 통과하면 된다고 생각하는 것도 위험하다.

결국 인지부하를 줄이는 기준은 생성된 코드의 양이 아니라 **사람이 새로 이해하고 판단해야 하는 범위**다. 코드 대신 믿을 수 있는 계약과 검증 방법을 얼마나 잘 쌓는지가 중요할 것 같다.
...
현재는 계약 기반 리뷰와 Stacked PR을 함께 쓰는 것이 무난해 보인다. 계약으로 확인할 수 있는 부분은 코드 전체를 읽지 않고, 구현 방식까지 확인해야 하는 부분은 한 번에 새로 생기는 개념의 수를 줄인다.

장기적으로는 사람이 코드를 더 빨리 읽는 방법보다, Agent가 계약을 기준으로 스스로 구현하고 검증해서 사람이 읽지 않아도 되는 범위를 넓히는 방법이 더 중요해질 것 같다.

AI가 구현 시간을 얼마나 줄였는지만 볼 때는 이 피로가 잘 보이지 않는다.

앞으로는 코드 생성 속도와 함께 **사람이 결과를 이해하고 승인하는 데 필요한 인지부하**도 개발 과정의 비용으로 다뤄야 할 것 같다.
...
[^4]: GitHub Docs, [About stacked pull requests](https://docs.github.com/en/pull-requests/collaborating-with-pull-requests/working-with-pull-requests/about-stacked-pull-requests) — 작은 의존 PR을 Stack으로 연결하고 각 PR의 변경을 독립적으로 리뷰하는 방식.
