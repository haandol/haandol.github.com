...
가상의 팀을 예로 들어보자. 모든 변경에 사람 검토가 필요하고, 사람은 하루 8개, 자동 검증은 하루 12개를 처리할 수 있다고 가정한다. `min`은 두 값 중 작은 값을 고른다는 뜻이므로, `min(8, 12) = 8`이다. 자동 검증이 12개를 끝내도 사람이 8개만 검토할 수 있으면 팀이 지속적으로 전달할 수 있는 변경은 하루 약 8개다.

그렇다면 사람이 하루 8개를 처리한다는 값은 어디서 나올까. 검토에 쓸 수 있는 시간과 변경 하나에 드는 시간을 알면 사람 쪽 용량을 계산할 수 있다.
...
앞의 팀이 하루 240분을 검토에 쓸 수 있고 변경 하나를 검토하는 데 평균 30분이 든다고 해보자. 모든 변경을 사람이 검토하므로 p는 1이다. 수식에 넣으면 `240 / (1 × 30) = 8`로, 하루 8개의 변경을 처리할 수 있다.

이번에는 전체 변경의 절반만 사람 검토가 필요해져 p가 0.5가 됐다고 해보자. H와 C가 그대로라면 `240 / (0.5 × 30) = 16`이다. 변경 16개 중 8개만 사람이 검토하므로 `8 × 30분 = 240분`이면 된다. 분모의 `p × C`는 전체 변경 하나당 평균적으로 필요한 사람 검토 시간인 셈이다.

따라서 이 값은 사람이 직접 검토한 건수가 아니라, 그 검토 여력으로 감당할 수 있는 전체 변경 수다. 다만 앞에서 자동 검증은 하루 12개까지 처리한다고 했으므로, 이 경우 팀 속도는 `min(16, 12) = 12`, 하루 약 12개로 제한된다.
...
이번에는 모든 변경을 사람이 검토하는 처음 조건으로 돌아가, 익숙해지는 비용을 줄여 15분이면 같은 수준으로 검토할 수 있다고 해보자. H는 240분, p는 1인 채 C만 줄어들어 `240 / (1 × 15) = 16`이 된다. 사람 검토 시간은 절반으로 줄고 감당할 수 있는 변경 수는 두 배가 된다. 이 경우에도 자동 검증의 용량이 그대로라면 팀 속도는 하루 약 12개까지 높아진다.
