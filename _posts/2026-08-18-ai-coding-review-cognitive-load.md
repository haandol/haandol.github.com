...
내 경험을 돌아보니 나는 그런 느낌을 거의 받지 못했던 것 같다. GitHub Copilot 베타 버전부터 도구를 사용하면서 작은 작업과 커밋, 짧은 피드백 루프에 익숙해졌기 때문인 것 같다.
...
인지부채는 시스템의 계약과 동작 방식을 설명하고 변경 영향을 판단하는 데 필요한 공유된 이해가 부족한 상태다. 내가 주목하는 경우는 이 부족함을 충분히 알아차리지 못한 채 코드를 반영하는 상황이다.

예를 들어 중복 결제를 막는 가상의 기능을 만들었다고 해보자. 화면에서 결제가 되고 재시도 테스트도 통과하지만, 기록이 사라진 뒤의 재시도나 두 요청이 동시에 들어오는 경우는 탐색하지 않았다.

이때 ‘같은 요청은 두 번 청구되지 않는다’는 계약을 아는 것과, 구현이 그 계약을 어떤 조건에서 지키는지 이해하는 것은 다르다. 단순히 동작한다는 결과만 보고 반영하면, 팀은 무엇을 놓쳤는지조차 모를 수 있다.
...
이 그림은 내가 설명하려는 발생 경로이지 모든 AI 개발에 적용되는 법칙은 아니다. 인지부채가 사라지고 기술부채로 바뀌는 것도 아니다. 이해가 부족한 상태와 그로 인해 잘못된 코드가 함께 남을 수 있다.

## 3. 팀 속도는 느린 검증 단계에 묶인다

코드 생성이 충분히 빨라졌다면 다음으로 볼 것은 검증의 처리 용량이다. Microsoft의 시스템 병목 설명도 지속 가능한 처리량에 관해 다음 원칙을 제시한다.[^27]
...
C에는 코드를 읽는 시간뿐 아니라 컨텍스트를 복원하고, 아키텍처와 문서의 가정을 확인하고, 예외를 탐색하는 시간이 포함된다. **인지부하가 팀 속도의 역수처럼 작용한다는 말은, 여기서는 이 사람 시간의 비용을 통해 해석한다.** 심리적인 부담 점수를 그대로 뒤집은 수식은 아니다.

예를 들어 하루 240분을 검토에 쓸 수 있고 모든 변경에 30분이 필요하다면 사람 쪽 용량은 하루 8개다. 익숙해지는 비용을 줄여 15분이면 같은 수준으로 검토할 수 있다면 16개가 된다. 다만 자동 검증의 용량이 하루 12개라면 팀은 그 이상을 지속적으로 통과시키지 못한다. 가상의 계산 예시다.

같은 인력과 가용 시간을 놓고 보면 사람 쪽에서 바꿀 수 있는 것은 C와 p다. 사람이 이해·판단하는 비용을 줄이거나, 사람의 직접 검토가 필요한 비율을 줄인다. 사람을 거치지 않는 경로는 p를 0으로 나눠 계산하지 않고, 사람 단계를 제외한 처리 용량으로 본다.
...
나는 구현 전에 계약을 정리하고, 결과에는 변경 전후의 동작, 실패 조건과 검증 근거를 함께 남기는 편이 낫다고 생각한다. 여기서 계약은 사용자에게 보여야 할 동작, 항상 지켜야 할 조건, 권한·데이터 경계처럼 구현의 성공 여부를 판정하는 기준이다.
...
이 그림은 일을 덜 한다는 뜻보다, 이해를 쌓는 시간을 작업 사이에 다시 배치하는 개념도다. 지금의 작은 변경에 익숙해진 상태가 다음 변경의 출발점이 되도록 만드는 것이다.

이미 쌓인 인지부채는 별도로 갚아야 한다. 함께 코드와 설계를 따라가며 놓친 가정과 실패 조건을 찾고, 잘못된 구현을 고치는 데는 시간이 든다. 그 집중까지 없애려는 것이 아니라, 다음 작업에서도 같은 맥락 복원과 과부하를 반복하지 않도록 투자하는 것이다.
...
같은 모듈 참조 방향을 반복해서 지적한다면 아키텍처 테스트로 옮긴다. 동일 요청의 중복 처리를 매번 확인한다면 회귀 테스트로 남긴다. 작업 지침, 도구와 검증 환경을 묶은 하네스에 이런 판단을 축적하면 다음 변경부터 재사용할 수 있다.[^5]
...
자동화 뒤에는 남은 사람 리뷰의 난이도도 다시 봐야 한다. 단순한 변경이 빠지고 어려운 판단만 남으면 p가 줄어도 C는 커질 수 있다. 효과는 실제 검토 시간과 품질로 확인하고, 비용 평가는 CTS-SW처럼 고객에게 전달한 단위를 기준으로 해야 한다.[^19]
