...
이때 놓치지 말아야 할 것은 다음 변경에 필요한 이해다. 리뷰 시간을 줄였더라도 팀에 동작을 설명할 사람이 남지 않는다면, 그 비용은 다음 수정이나 장애 대응에서 다시 나타날 수 있다.
...
리뷰가 밀렸을 때 단순히 읽는 시간을 줄이면 어떻게 될까.
...
여기에 빨리 승인해야 한다는 압박까지 더해지면, 동작을 끝까지 따라가기보다 테스트 통과 표시나 Agent의 요약을 판단 근거로 삼기 쉬워진다고 생각한다. 이때 코드에 문제가 없는 것과 아직 문제를 찾아보지 못한 것이 구별되지 않는다.
...
| 인지부채 | 팀이 공유하는 시스템 동작에 대한 이해 | 실패 뒤 재시도했을 때 어떻게 되는지 설명할 사람이 있는가? |
| 의도부채 | 밖으로 기록하지 않은 목적, 제약과 결정 이유 | 왜 이 동작을 골랐고 어떤 조건을 지켜야 하는지 남아 있는가? |

지금 리뷰할 때 드는 부담이 인지부하라면, 이해하지 못한 채 넘긴 변경 때문에 이후 작업에 남는 부담이 인지부채라고 볼 수 있다. 어려운 코드를 읽는다고 항상 부채가 생기는 것은 아니다. 그 과정에서 이해를 얻었다면 다음 작업의 부담을 줄일 수도 있다.

반대로 문서를 작성했다고 인지부채가 없어지는 것도 아니다. 결정 이유를 기록하면 의도부채를 줄일 수 있지만, 팀이 그 내용을 이해하고 다음 판단에 사용할 수 있는지는 별도로 봐야 한다.
...
이해의 빈칸이 잘못된 변경을 통과시키고, 이미 존재하던 결함의 발견을 늦추기도 한다. 이런 경로로 인지부채가 기술부채를 만들거나 키울 수 있다고 생각한다.

{% raw %}
```mermaid
flowchart TD
    A["리뷰할 변경 누적"] --> B["시간 압박<br/>검토 생략"]
    B --> C["인지부채<br/>동작·제약 이해 부족"]
    D["의도부채<br/>목적·결정 이유 누락"] --> C
    C --> E["다음 변경에서<br/>조건을 놓침"]
    E --> F["기술부채 증가<br/>잘못된 변경·수정 비용"]
    F --> C
    A -. "대응" .-> G["작게 나눠<br/>이해·검증"]
    G --> H["조건·이유 기록<br/>실패 경로 테스트"]
    H --> I["다음 변경의<br/>판단 근거"]
```
{% endraw %}

이 그림은 연구 하나가 입증한 필연적인 순서가 아니라, 부채가 서로 영향을 주는 경로를 정리한 개념도다. 이해하지 못한 코드에도 결함이 없을 수 있고, 테스트와 정적 분석으로 결함을 먼저 발견할 수도 있다.
...
팀이 바꿀 수 있는 것은 크게 두 가지다. **사람에게 들어오는 검토 수요를 줄이거나, 필요한 검토를 더 적은 노력으로 끝내도록 돕는다.**

앞 절의 계약과 테스트는 전자에, 맥락 설명과 공동 리뷰는 후자에 기여할 수 있다. DORA도 AI가 만든 큰 변경에 대응하려면 작은 작업 단위, 작성 단계의 자동 검사와 검증 방식의 재검토가 필요하다고 설명한다.[^14]
