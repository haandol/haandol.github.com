...
기존 ML 모델들과 달리 그래프 자체를 어느정도 이해하지 않으면 GNN 을 쓰기가 어려워서 그래프 쪽을 공부하다보니 2주가 지나버렸고, 목적대비 시간을 너무 많이 쓴것 같았다.

이 기회에 글을 쓰면서 머리에 정리를 해보면서 마무리 하려고 한다.
...
본 글에서는 GNN 을 시작하기 전에 알아두면 좋은 내용들을 정리하고자 한다.
