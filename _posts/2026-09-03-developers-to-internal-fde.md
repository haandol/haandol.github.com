...
- 외부 인력이 떠난 뒤에도 업무 절차를 고칠 사람이 필요하다.
- 기존 개발자의 다음 역할 중 하나로 사내 FDE를 생각한다.

## 시작하며

최근 기존 개발자가 앞으로 어떤 일을 하게 될지 자주 생각했다.

코드를 작성하고 실행하는 Coding Agent로 구현 작업을 자동화해나가면 같은 기능을 만드는 데 필요한 개발자의 시간은 줄어든다. 더 어려운 설계와 리뷰에 시간을 쓸 수도 있고, 더 많은 기능을 만들 수도 있다.
...
요즘은 기존 개발자가 FDE 역할로 옮겨갈 수도 있겠다는 생각이 들어, 실제로 어떤 일을 하는지 발표와 사례를 찾아보고 있다.
...
Agent가 한 번 작업을 끝내는 것만으로는 부족하고, 실패를 관찰해 현업 전문가의 수정을 표준 업무 절차(SOP)와 하네스에 계속 반영해야 한다는 뜻이다.

여기서 하네스는 Agent에게 주는 배경 정보와 권한, 실행 환경, 결과를 평가하고 위험한 실행을 막는 장치를 통틀어 말한다. 이전에 정리한 [하네스 엔지니어링](/2026/03/15/harness-engineering-beyond-context-engineering.html)을 기업 업무에 적용한 모습에 가깝다.

발표자는 200개가 넘는 Agent 실행 인스턴스를 운영하면서 9개월 동안 10만 건이 넘는 수정 사항(correction)을 쌓았다고 설명한다. 국가마다 다른 절차를 담은 SOP 모음의 규모는 Agent 실행 코드의 약 20배였고, 실패 유형 하나를 제대로 해결하는 데 한두 달이 걸리기도 했다고 한다.[^2]
...
각 시스템이 정상적인 화물을 잘 처리하더라도, 상태가 서로 어긋나면 담당자는 여러 화면과 이메일을 돌아다니며 실제 상태를 확인하고 어느 팀에 무엇을 요청해야 하는지 판단해야 한다.
...
이 사례를 바탕으로 내가 정리한 FDE의 작업은 다음과 같다.

| 단계 | 실제 작업 | 남는 것 |
| --- | --- | --- |
| 현장 관찰 | 담당자가 예외를 처리하는 과정과 판단 기준을 확인한다 | 현재 업무 흐름과 도입 전 상태 |
| 범위 결정 | 첫 자동화 범위와 사람이 판단할 범위를 정한다 | 자동화할 업무와 사람이 맡을 업무 |
| 업무 표현 | 담당자만 알고 있던 판단을 실행 가능한 절차로 옮긴다 | SOP와 하네스 |
| 사전 검증 | 과거 사례를 읽기 전용으로 다시 실행한다 | 평가 사례와 실패 유형 |
| 제한적 운영 | 실제 변경 없이 제안하는 Shadow Mode부터 시작한다 | 실행 기록(Trace)과 현업의 수정 |
| 반복 개선 | 실패를 하네스에 반영하고 안정된 절차를 코드로 옮긴다 | 여러 단계를 묶은 도구와 축적된 업무 지식 |
...
이런 수정이 다음 실행에 도움이 되려면 `"결과가 별로다"`라는 피드백에 머물러서는 안 된다. 특정 국가와 상태에서는 어느 절차를 먼저 실행해야 한다는 식으로 실제 동작을 바꿔야 한다. 같은 실패를 다시 확인할 평가 사례도 함께 남기는 편이 좋다고 생각한다.
...
처음에는 Agent가 운송 예약(Booking), 결제, 세관 서류를 어떤 순서로 확인할지 판단하는 편이 유리할 수 있다. 업무 자체를 충분히 이해하지 못했고 새로운 예외도 계속 나오기 때문이다.

운영을 반복하면서 항상 같은 조건에서 같은 순서로 처리되는 부분이 보이기 시작한다.

예를 들어 `Booking 조회 → 결제 확인 → 서류 확인 → 결과 검증` 순서가 안정적으로 성공한다면, 매번 Agent가 다음 단계를 추론하게 둘 이유가 줄어든다. 이 절차를 하나의 함수나 API로 묶어 `resolve_booking_exception()`처럼 호출하게 만들 수 있다. 발표에서는 이렇게 여러 단계를 묶어 재사용하는 도구를 Composite Tool이라고 부른다.[^5]

FDE가 입력과 출력, 실패 조건과 검증 기준을 정하면 Coding Agent가 이 도구와 테스트를 작성할 수 있다. 반복해서 성공한 절차의 실행 순서와 분기 조건을 코드에 고정하는 과정이다.

| Agent가 매번 절차를 결정 | 절차를 코드로 묶은 Composite Tool |
| --- | --- |
| 실행할 때마다 단계와 분기를 판단한다 | 실행 순서와 분기 조건이 코드에 고정된다 |
| 같은 입력에서도 경로가 달라질 수 있다 | 같은 조건에서 같은 제어 흐름을 따른다 |
| 각 도구의 테스트와 함께 Agent가 고른 업무 흐름을 평가한다 | 정해진 분기와 결과를 단위·통합 테스트로 확인한다 |
| Agent가 선택할 수 있는 각 도구의 권한을 확인한다 | 묶은 절차 전체에 필요한 권한을 확인한다 |
| 실행 기록을 따라 Agent가 고른 경로와 실패 원인을 확인한다 | 코드에 정한 경로와 실행 기록을 함께 확인한다 |

외부 시스템의 데이터와 응답은 달라질 수 있지만, 조회 순서와 분기, 검증과 복구 규칙은 코드에 남는다. 같은 입력과 외부 응답을 준비해 오류가 다시 발생하는지 테스트하고, 어느 단계에서 어떤 권한을 쓰는지도 확인하기 쉬워진다.
...
예를 들어 고객 문의를 처리하기 위해 담당자가 Excel을 확인하고, 다른 팀에 메시지를 보내고, 승인을 기다린 뒤 주문과 재무 정보를 관리하는 전사 자원 관리 시스템(ERP)에 입력한다고 해보자.

필요한 접근 권한과 연동 수단을 마련하면 Excel 조회와 메시지 전송, ERP 입력을 Agent에게 맡길 수 있다.

하지만 왜 승인을 기다려야 하는지, 왜 필요한 데이터가 Excel에 있는지, 왜 시스템마다 값이 다른지는 그대로 남는다. Agent가 모르는 경우에는 다시 사람에게 전달하면 시스템도 일단 돌아간다.

초기에는 괜찮아 보여도, 시간이 지나면 금액이 클 때는 팀장, 특정 거래처는 대금 지급 담당자, 유럽연합(EU) 고객은 규정 준수 담당 조직으로 보내는 규칙이 하나씩 붙을 수 있다. 새로운 요구사항을 코드로 추가하는 비용이 낮아진 만큼 이런 규칙도 전보다 빨리 늘어날 수 있다.
...
몇 년 동안 이런 변경이 쌓이면 업무 규칙이 코드뿐 아니라 하네스 안의 Prompt와 SOP, 평가 사례, 사람에게 넘기는 조건으로 흩어진다. 코드만 봐서는 전체 흐름을 이해하기 어려워진다.
...
그런데 1만8천 건을 분류해보니 데이터 부족이 8천 건, 정책이 모호한 경우가 4천 건, 시스템의 값이 다른 경우가 3천 건이고 실제로 사람의 판단이 필요한 건 3천 건일 수 있다.
...
```mermaid
flowchart LR
    A["영업<br/>고객 관리 시스템"] --> B["계약<br/>계약 시스템"]
    B --> C["주문<br/>주문 시스템"]
    C --> D["물류<br/>물류 시스템"]
    D --> E["재무<br/>ERP"]
    E --> F["정산<br/>정산 시스템"]
...
각 시스템의 권한과 데이터 정의, 성과 지표를 서로 다른 조직이 관리할 수 있다.

[이벤트 스토밍 글](/2020/12/10/demystifying-event-storming.html)을 쓸 때도 업무 전문가들의 지식이 부서별로 흩어져 있어 전체 업무를 파악한 사람이 없다고 정리했다.

여러 실무자가 모여 각자 아는 업무를 연결하면 전체 흐름을 그릴 수 있다. 하지만 다른 조직의 데이터에 접근하고, 어느 시스템을 기준으로 삼을지 정하고, 조직 사이의 책임을 바꾸려면 이를 결정할 권한을 가진 사람이 참여해야 한다.

FDE는 문제를 찾고 해결안을 만들 수 있지만 그 권한을 스스로 만들 수는 없다.

특히 한국 기업에서 흔히 보는, 부서마다 데이터와 의사결정을 따로 관리하는 구조를 생각하면 이 부분이 가장 어렵다. 자동화해야 하는 업무는 여러 부서를 지나가는데 FDE가 실제로 바꿀 수 있는 범위는 요청한 부서와 시스템 안에 머무르기 쉽다.
...
업무 전체 결과를 책임지는 담당자(Process Owner)와 데이터 접근, 조직 간 책임 변경을 밀어줄 경영진이 함께 있어야 한다. 역할을 나누면 다음과 같다.
...
| 사내 FDE | 실행 기록 분석, 하네스 개선과 시스템 반영 |

FDE는 운영 데이터와 실행 기록으로 문제를 보여주고, 합의된 변경을 실제 시스템에 반영한다.
...
외부 FDE에게 초기 구축과 전문 기술을 맡기더라도, 운영 중 같은 실패가 반복되면 어느 업무 절차를 어떻게 바꿀지는 내부에서 결정해야 한다.

이 역할까지 외부에 맡기면 계약이 끝날 때 개선도 함께 멈출 가능성이 크다.

## 5. 기존 개발자의 다음 역할로 FDE를 생각해본다
...
이 설명을 들으면서 기존 개발자가 FDE로 역할을 넓힐 수 있겠다는 생각이 조금 더 구체화됐다.

내부 시스템을 오래 개발하고 운영한 사람은 어디에서 자주 문제가 생기는지, 데이터가 실제로 어디에 있는지, 문서와 현실이 어떻게 다른지 알고 있다. 같은 조직에서 일하면서 어느 팀과 어떤 순서로 이야기해야 하는지도 어느 정도 알게 된다.
...
개발자가 업무 개선까지 맡으면 구현에 더해 다음과 같은 판단을 해야 한다.

| 구현에 집중할 때 | 업무 개선까지 맡을 때 |
| --- | --- |
| 전달받은 요구사항을 구현한다 | 요구사항이 필요한 이유부터 확인한다 |
| 담당 시스템의 범위를 본다 | 업무가 시작하고 끝나는 전체 경로를 본다 |
| 요구사항대로 배포됐는지 확인한다 | 운영에서 반복되는 예외까지 따라간다 |
...
Palantir는 Foundry의 관리와 운영 작업을 자연어로 수행하는 Agent를 `AI FDE`라는 이름으로 제공하고 있다.[^4] 회의 기록을 정리하고, 실패한 작업의 실행 기록을 묶고, SOP 초안과 시스템 연동 코드를 만드는 일도 Agent가 점점 더 많이 맡을 수 있다.
...
처음에는 Coding Agent가 발전하면 기존 개발자가 설계와 리뷰를 더 많이 맡게 될 것이라고 생각했다. 지금은 현업의 요구를 애플리케이션으로 옮기던 개발자가 업무와 Agent, 조직 변화를 함께 다루는 FDE 형태로 발전할 수도 있겠다는 생각이 든다.
...
[^1]: OpenAI, [Forward Deployed Engineer](https://openai.com/careers/forward-deployed-engineer-san-francisco/) — 고객의 문제 발견부터 프로덕션 배포까지 함께 책임지는 FDE 역할을 설명한다.
